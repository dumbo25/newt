...
# To Do List:
# ??? **************************** STOPPED HERE ******************** ???
#   b) django.sh shouldn't require that install.sh has been run
#      add to django.cfg andything that install.cfg has that django.sh requires
#   c) create a shell script for apps
#      c.1) need to add urls.py to each app
#       nano /home/pi/newt_site/urls.py or is it newt_site/newt_site
#      c.2) app.urls
#      c.3) get beyond rocket ship django demo page
...
# ??? **************************** STOPPED HERE ******************** ???
#   j) update help in django.sh
#      j.1) ensure options for django.sh work
# ??? *************************** WORKING ON THIS *******************????
#      j.2) add option to work w/ or w/o virtual en
#		need to ensure v_$DjangoProject still works if VirtualEnv = false
#		django.cfg ApacheConfig needs work
#      MUST OUTPUT DJANGO DIRECTORY - in one casse it is in project and in other it is ??? .local/bim
#      j.3) add other options
#      j.4) move global variables into django.sh
#      j.5) identify the editable variables in help
#      j.6) create google sites page explaing how to set up django using django.sh
...
#   r) move from http 80 to https 443
#
#   x) run shell check
...
function installDjango {
    ...
    cd "$BaseDirectory/$DjangoProject$VirtualDirectory"
    pwd
...
function getSettings {
    ...
    then
        ...
        sed -i "s/ALLOWED_HOSTS.*/$var/" "$BaseDirectory/$DjangoProject/p_$DjangoProject/p_$DjangoProject/settings.py"
    fi
}
...
function getApacheConf {
    ...
    then
        ...
        daemon="$DjangoProject python-path=/home/pi/$DjangoProject python-home=/home/pi/$DjangoProject$VirtualDirectory"
        ...
        daemon="$DjangoProject python-path=/home/pi/$DjangoProject python-home=/home/pi/$DjangoProject/p_$DjangoProject"
...
do
    case $option in
        ...
        v) # do not add or run from virtual env
            VirtualEnv=false
            VirtualDirectory=""
            NoVirtualEnvDjangoDirectory="/home/pi/.local/bin/"
            ;;
...
echo "DEBUG: before getApacheConf"
getApacheConf
...
ls
    cd "$BaseDirectory/$DjangoProject"
    source "v_$DjangoProject/bin/activate"
echo "DEBUG: superuser venv = true"
pwd
ls
    # create useer
...
echo "DEBUG: superuser venv = false"
pwd
ls
